...
aidlc engine state lookup agent-for SLUG          # → lead agent name
aidlc engine state lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
```bash
aidlc engine state reuse-artifact <stage-slug> \
